...
import NewsService from '../../domain/services/new.service';
...
class NewsController {
  ...
  async getAllNews(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ message: "Failed to retrieve news items", error: (error as Error).message });
    }
  }
  ...
  async getNewsById(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ message: "Failed to retrieve news item", error: (error as Error).message });
    }
  }
  ...
  async updateNews(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { title, description } = req.body;
      const updatedNews = await NewsService.updateNews(id, { title, description });
      if (updatedNews) {
      ...
      res.status(500).json({ message: "Failed to update news item", error: (error as Error).message });
    }
  }
  ...
  async deleteNews(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ message: "Failed to delete news item", error: (error as Error).message });
    }
  }
}
