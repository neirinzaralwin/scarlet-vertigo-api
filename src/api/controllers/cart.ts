...
class CartController {
  ...
  async updateCartItem(req: Request, res: Response): Promise<void> {
    try {
      ...
      const host = req.headers.host;
      const links = [
        { rel: "view_cart", href: `http://${host}/carts`, method: "GET", description: "View your cart" },
        { rel: "add_to_cart", href: `http://${host}/carts`, method: "POST", description: "Add item to cart" },
        { rel: "empty_cart", href: `http://${host}/carts/empty`, method: "DELETE", description: "Empty the cart" },
        { rel: "remove_item", href: `http://${host}/carts`, method: "DELETE", description: "Remove item from cart using body id" },
        { rel: "update_quantity", href: `http://${host}/carts`, method: "PATCH", description: "Update item quantity in cart using body id" }
      ];
  
      res.status(200).json(new ApiResponse(200, { cart: updatedCart, links }, "Cart updated successfully"));
    } catch (error) {
      if (error instanceof ApiError) {
        res.status(error.statusCode).json(new ApiResponse(error.statusCode, null, error.message));
      } else {
  ...
  async removeItemFromCarts(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!userId || !Types.ObjectId.isValid(userId) || !Types.ObjectId.isValid(productId)) {
      ...
      res.status(200).json(new ApiResponse(200, { cart, links }, "Item removed from cart successfully"));
    } catch (error) {
      if (error instanceof ApiError) {
        res.status(error.statusCode).json(new ApiResponse(error.statusCode, null, error.message));
      } else {
